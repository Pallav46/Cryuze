...
const Order = () => {
  ...
  const { createdAt, serviceProvider, status, subCategory, updatedAt, bill } =
    data.data;
  ...
  const handlePayUsingRazorpay = async (e) => {
    ...
    try {
      const {
        data: { key },
      } = await axios.get("/api/v1/pay/getKey");
  
      const {
        data: { order },
      } = await axios.post("/api/v1/pay/checkout", {
        amount, orderId
      });
        ...
        handler: async function (response) {
          ...
          try {
            const validateRes = await fetch(
              "/api/v1/pay/paymentverification",
              {
                method: "POST",
  ...
  const totalAmount =
    (bill?.subcategoryCharge || 0) +
    (bill?.serviceCharge || 0) +
    (bill?.additionalCharges?.reduce((acc, charge) => acc + charge.price, 0) ||
      0);
